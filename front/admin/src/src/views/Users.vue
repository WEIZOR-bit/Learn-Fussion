            ...
            <div class="d-flex justify-content-center mt-3">
              <soft-pagination color="success" size="sm">
                <soft-pagination-item
                    @click="users.current_page > 1 && fetchUsers(users.current_page - 1)"
                ...
                <soft-pagination-item
                    ...
                    :label="page"
                    :class="{'disabled': users.current_page === page}"
                    :active="users.current_page === page"
                />
                <soft-pagination-item
                    @click="(users.current_page < users.last_page && users.total > 0) && fetchUsers(users.current_page + 1)"
                    :disabled="users.current_page === users.last_page || users.total === 0"
                    :class="{'disabled': users.current_page === users.last_page || users.total === 0}"
                    next
                />
              </soft-pagination>
            </div>
  ...
  setup() {
    ...
    const searchUsers = async (page = 1) => {
      ...
      if (searchQuery.value === '') {
        ...
        try {
          await userStore.search(page, searchQuery.value);
        } catch (err) {
          console.error(err);
        } finally {
          isLoading.value = false;
